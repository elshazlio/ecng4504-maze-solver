...
const unsigned long ALIGN_STABLE_MS       = 25;
...
const char* stateName(RobotState s)
{
  switch (s) {
    ...
    case PAUSED_ARM_TRAINING: return "PAUSED_ARM_TRAINING";
    case PAUSED_TRAINING:     return "PAUSED_TRAINING";
    case PAUSED_ARM_SOLVING:  return "PAUSED_ARM_SOLVING";
    case PAUSED_SOLVING:      return "PAUSED_SOLVING";
...
void reportPaths()
{
  ...
  if (rawPathLen == 0 && optimalPathLen == 0) {
    logLine("PATH_EMPTY");
  }
}
...
void buildOptimalPath()
{
  ...
  for (int i = 0; i < rawPathLen; i++) {
    ...
    // reduce x B y whenever it appears at the tail
...
bool turnExists(char requested, bool foundLeft, bool foundStraight, bool foundRight)
{
  switch (requested) {
    ...
    case 'B': return true;   // always physically possible as a recovery move
...
void handleTrainingDeadEndRecovery()
{
  ...
  doTurn('B');                // U-turn until line is found again
  recordNode(0, 'B', segMs);  // record as dead end, not line loss
...
void handleSolvingDeadEndRecovery()
{
  ...
  doTurn('B');   // recover by U-turning back to the line
...
void inspectAndHandleNodeTraining()
{
  ...
  // segment time is from previous stored decision node to THIS decision node
  unsigned int segMs = (unsigned int)(millis() - segmentStartMs);

  // only real decision nodes are stored:
  //   - intersections with >=2 choices
  //   - dead ends (B)
  // forced bends are executed but not stored
  if (choices >= 2 || turnTaken == 'B') {
    ...
    doTurn(turnTaken);
    // segmentStartMs intentionally not reset for forced bends
  }
...
void handleTraining()
{
  ...
  // persistent no-line during training is treated as a dead end
  if (persistentNoLine()) {
...
void handleSolving()
{
  ...
  // persistent no-line during solving is recovered using a U-turn,
  // not generic lost-line search behavior
  if (persistentNoLine()) {
...
void updateLedPattern()
{
  switch (state) {
    ...
    case PAUSED_ARM_TRAINING:
    case PAUSED_TRAINING:
    case PAUSED_ARM_SOLVING:
    case PAUSED_SOLVING:
...
void enterPaused(RobotState pausedState, const char *pausedStageName)
{
  strncpy(resumeStage, currentStage, sizeof(resumeStage) - 1);
  resumeStage[sizeof(resumeStage) - 1] = '\0';
  stopMotors();
  state = pausedState;
  setStage(pausedStageName);
  logLine(String("STATE=") + stateName(state));
  logLine("CMD=PAUSE");
}

void resumeFromPaused(RobotState targetState)
{
  if (resumeStage[0] == '\0') {
    logLine("RESUME_FAILED");
    return;
  }
  state = targetState;
...
void processCommand(char *cmd)
{
  ...
  if (strcmp(cmd, "PAUSE") == 0) {
    switch (state) {
      case ARM_TRAINING:
        enterPaused(PAUSED_ARM_TRAINING, "PAUSED_ARM_TRAINING");
        return;
      case TRAINING:
        enterPaused(PAUSED_TRAINING, "PAUSED_TRAINING");
        return;
      case ARM_SOLVING:
        enterPaused(PAUSED_ARM_SOLVING, "PAUSED_ARM_SOLVING");
        return;
      case SOLVING:
        enterPaused(PAUSED_SOLVING, "PAUSED_SOLVING");
        return;
      default:
        logLine("PAUSE_IGNORED");
        return;
    }
  }
  ...
  if (strcmp(cmd, "RESUME") == 0) {
    switch (state) {
      case PAUSED_ARM_TRAINING:
        resumeFromPaused(ARM_TRAINING);
        return;
      case PAUSED_TRAINING:
        resumeFromPaused(TRAINING);
        return;
      case PAUSED_ARM_SOLVING:
        resumeFromPaused(ARM_SOLVING);
        return;
      case PAUSED_SOLVING:
        resumeFromPaused(SOLVING);
        return;
      default:
        logLine("RESUME_IGNORED");
        return;
    }
  }
...
void handleCommandStream(Stream &port, char *buffer, uint8_t &indexRef)
{
  while (port.available()) {
    ...
    if (indexRef < (CMD_BUF_SIZE - 1)) {
      ...
      indexRef = 0; // overflow protection
    }
  }
}
...
void setup()
{
  ...
  logLine("Bluetooth commands: TRAIN, START, STATUS, PATH, DUMP, LOAD, CLEAR, STOP, PAUSE, RESUME");
